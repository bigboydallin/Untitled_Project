class GameScene extends Phaser.Scene {
    ...
    update() {
        for (let i = 0; i < gameState.layers; i++) {
          gameState.displays[i].setText(i + ': ' + gameState.currencys[i].count + '/ ' + gameState.currencys[i].max);
          gameState.stars[i].setScale(gameState.currencys[i].ratio,gameState.currencys[i].ratio);
        }
        gameState.currencys[1].increment(5);
        gameState.currencys[2].increment(1);
    }
}
